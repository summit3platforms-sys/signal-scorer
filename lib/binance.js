...
async function withRetry(fn, maxRetries = 3) {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      logError('Binance API', `Transient request failed (attempt ${attempt + 1}/${maxRetries}): ${err.message}`, err.stack);
...
export async function getAllUSDTFuturesPairs() {
  ...
  return binanceBreaker.call(() => withRetry(async () => {
    ...
    const realWeight = parseInt(res.headers['x-mbx-used-weight-1m'] || '0', 10);
    if (realWeight > 0) usedWeight = realWeight;
    ...
    await Promise.allSettled(
      batch.map(async symbol => {
        try {
          ...
          logError('Binance API', `Klines batch fetch failed for ${symbol}: ${err.message}`, err.stack);
        }
      })
    );
...
// Funding rates cache — TTL 5 minutes (rates only change every 8 hours)
let fundingCache = { data: new Map(), expires: 0 };

export async function getFundingRates() {
  if (fundingCache.expires > Date.now()) return fundingCache.data;
  ...
  return binanceBreaker.call(() => withRetry(async () => {
    ...
    for (const d of res.data) {
      ...
      map.set(d.symbol, {
        symbol: d.symbol,
        rate,                                    // raw 8h rate e.g. 0.0003
        ratePct: rate * 100,                     // percentage e.g. 0.03%
        annualized: rate * 3 * 365 * 100,        // annualized %
        markPrice: parseFloat(d.markPrice),
        nextFundingTime: parseInt(d.nextFundingTime)
      });
    }

    fundingCache = { data: map, expires: Date.now() + 5 * 60_000 };
    console.log(`[Binance] Funding rates fetched for ${map.size} symbols`);
    return map;
  }));
}
