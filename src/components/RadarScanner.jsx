...
// ── Sidebar metric row (vertical mode) ────────────────────────────────────────
...
// ── Mobile metric mini-card (grid mode) ───────────────────────────────────────
function MetricMini({ label, value, unit, color, bar }) {
  return (
    <div style={{ padding: '6px 8px', background: 'rgba(0,0,0,0.4)', borderRadius: 8, border: `1px solid ${color}18`, position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 1, background: `linear-gradient(90deg,transparent,${color}44,transparent)` }} />
      <div style={{ fontSize: 7.5, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'monospace', marginBottom: 2 }}>{label}</div>
      <div style={{ fontSize: 16, fontWeight: 800, color, fontFamily: 'monospace', textShadow: `0 0 8px ${color}88`, lineHeight: 1 }}>
        {value}<span style={{ fontSize: 8, color: '#4b5563', fontWeight: 400 }}>{unit}</span>
      </div>
      <div style={{ height: 2, background: 'rgba(255,255,255,0.04)', borderRadius: 2, marginTop: 4 }}>
        <div style={{ height: '100%', width: `${bar}%`, background: `linear-gradient(90deg,${color}44,${color})`, borderRadius: 2, transition: 'width 0.9s ease' }} />
      </div>
    </div>
  );
}

// ── Shared animated state hook ────────────────────────────────────────────────
function useRadarState() {
  const ORBIT_R = 92;

  const [tickers,      setTickers]      = useState([]);
  const [metrics,      setMetrics]      = useState(() =>
    ALL_METRICS.map(m => ({ ...m, value: randInt(m.min, m.max), bar: randInt(30, 90) }))
  );
  ...
  return { tickers, metrics, pairsScanned, ORBIT_R };
}

// ── CSS shared across both layouts ────────────────────────────────────────────
const SHARED_STYLES = `
...
// ── Radar display widget (canvas + tickers) ───────────────────────────────────
function RadarWidget({ size, tickers, ORBIT_R }) {
  const cx = size / 2, cy = size / 2;
  return (
    <div className="relative flex-shrink-0" style={{ width: size, height: size }}>
      {[0, 0.85, 1.7].map((delay, i) => (
        <div key={i} className="radar-sb-pulse-ring" style={{ width: size * 0.6, height: size * 0.6, top: '50%', left: '50%', animationDelay: `${delay}s` }} />
      ))}
      <div className="absolute" style={{ top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }}>
        <RadarCanvas size={size * 0.88} />
      </div>
      {tickers.map(t => (
...
function MobileRadar({ tickers, metrics, pairsScanned, ORBIT_R }) {
  ...
  return (
    <div className="relative overflow-hidden" style={{
      ...
      {/* Header */}
      <div className="flex items-center justify-between px-3 pt-2.5 pb-2" style={{ borderBottom: '1px solid rgba(0,255,136,0.07)' }}>
        <div className="flex items-center gap-1.5">
          ...
          <span style={{ fontFamily: 'monospace', fontSize: 8.5, fontWeight: 700, color: '#00ff88', letterSpacing: '0.14em' }}>MARKET SCAN RADAR</span>
        </div>
        <span style={{ fontFamily: 'monospace', fontSize: 8, color: '#374151' }}>{pairsScanned}/150 PAIRS</span>
      </div>

      {/* Body: radar left + metrics grid right */}
      <div className="flex items-center gap-3 p-3">
        {/* Radar */}
        <RadarWidget size={RADAR_SIZE} tickers={tickers} ORBIT_R={ORBIT_R} />

        {/* Metrics grid — 2 columns, all 7 metrics */}
        <div className="flex-1 grid grid-cols-2 gap-1.5 min-w-0">
          {metrics.map(m => (
            <MetricMini key={m.label} label={m.label} value={m.value} unit={m.unit} color={m.color} bar={m.bar} />
          ))}
        </div>
      </div>

      {/* Footer */}
      <div className="px-3 py-1.5" style={{ borderTop: '1px solid rgba(0,255,136,0.05)', fontSize: 7.5, fontFamily: 'monospace', color: '#1f2937', textAlign: 'center', letterSpacing: '0.08em' }}>
        &#x25CF; 6-FACTOR &middot; MTF 15M/1H/4H &middot; ATR TARGETS &middot; LIVE
      </div>
    </div>
  );
}
...
function SidebarRadar({ tickers, metrics, pairsScanned, ORBIT_R }) {
  const CONTAINER = 220;
  return (
    <div className="relative overflow-hidden" style={{
      background: 'linear-gradient(160deg,#020a12 0%,#050d18 60%,#021510 100%)',
      ...
      <div className="flex justify-center py-3 relative" style={{ background: 'rgba(0,0,0,0.2)' }}>
        <RadarWidget size={CONTAINER} tickers={tickers} ORBIT_R={ORBIT_R} />
      </div>
...
// ── Main export — picks layout based on `mobile` prop ────────────────────────
export default function RadarScanner({ mobile = false }) {
